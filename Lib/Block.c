...
char* block_to_str(Block* block){
  ...
  char str[11]; //11 car un int en char au maximum prend 11 de place.
  char *auteur =key_to_str(block->author);
  char *finalchar=(char*)calloc(sizeof(char),10000);//On alloue 10000 de place pour être large 
...
void compute_proof_of_work(Block *B, int d) {
  ...
  // on fait une boucle while pour compter le nombre de d consecutive au debut de valeurhache
  int compteur = 0;
  while (compteur < d) {
